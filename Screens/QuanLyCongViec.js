...
import { Modal, StyleSheet, Text, View, Image, ScrollView, Pressable, FlatList, Alert, Platform, TextInput } from 'react-native'
...
const onChangeNgayBatDau = (event, selectedDate) => {
  const currentDate = selectedDate || date;
    ...
    let fDatengayBatDau = tempDatengayBatDau.getFullYear() + '-' + (tempDatengayBatDau.getMonth() + 1) + '-' + tempDatengayBatDau.getDate();
...
const onChangeNgayKetThuc = (event, selectedDate) => {
 ...
 setShow(false);
  ...
  let fDatengayKetThuc = tempDatengayKetThuc.getFullYear() + '-' + (tempDatengayKetThuc.getMonth() + 1) + '-' + tempDatengayKetThuc.getDate();
...
const showMode = (currentMode) => {
  ...
  const getList_cv = async () => {
    try {
      const response = await axios.get('http://192.168.2.102:3000/congviecs/getCongViec');
  ...
  const putList_nv = async () => {
    ...
    const obj = {ngayBatDau, trangThai,noiDungCongViec,tenCongViec, ngayKetThuc  };

    try {
      await axios.put(`http://192.168.2.102:3000/congviecs/updateCongViec/${id}`, obj);
...
<Text style={{textAlign:'center',color:'#fff',fontWeight:'bold'}}>Từ Ngày</Text>
</View>
...
<Text style={{width:72}}>{getFormattedDate(ngayKetThuc)}</Text>
      <Pressable  onPress={() => showMode('date')}>
<View style={{width:90,borderRadius:20,height:40,padding:'5%',justifyContent:'center',backgroundColor:'#DAAE7B'}}>
<Text style={{textAlign:'center',color:'#fff',fontWeight:'bold'}}>Đến Ngày</Text>
</View>
    ...
    height:100
          ...
          <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-around', alignItems: 'center', }}>
            <Pressable style={{ width: 100, height: 50, backgroundColor: '#B0A4A8', justifyContent: 'center', borderRadius: 25, alignItems: 'center' }} onPress={() => setModalVisible_addcv(false)}>
              <Text style={{ color: 'black', textAlign: 'center' }}>Đóng</Text>
            </Pressable>
            <Pressable style={{ width: 100, height: 50, backgroundColor: '#B0A4A8', justifyContent: 'center', borderRadius: 25, alignItems: 'center' }} onPress={postList_cv}>
...
<Text style={{textAlign:'center',color:'#fff',fontWeight:'bold'}}>Đến Ngày</Text>
  ...
  onValueChange={(newTrangThai) => {
    setTrangThai(newTrangThai ? 1 : 0);
          ...
          <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-around', alignItems: 'center', }}>
            <Pressable style={{ width: 100, height: 50, backgroundColor: '#B0A4A8', justifyContent: 'center', borderRadius: 25, alignItems: 'center' }} onPress={() => setModalVisible_editcv(false)}>
 ...
 <Text style={{ marginTop: 15, fontSize: 19, color: 'black', fontWeight: 'bold', paddingLeft: '5%' }}>Ngày bắt đầu</Text>
            ...
            <Text style={{ fontSize: 16, color: 'black', fontWeight: '500', paddingLeft: '5%' }}>  {getFormattedDate(congViec_id.ngayKetThuc)}</Text>

            <Text style={{ marginTop: 15, fontSize: 19, color: 'black', fontWeight: 'bold', paddingLeft: '5%' }}>trạng thái</Text>
            <Text style={{ fontSize: 16, color: 'black', fontWeight: '500', paddingLeft: '5%' }}>  {congViec_id.trangThai}</Text>

            <Text style={{ marginTop: 15, fontSize: 19, color: 'black', fontWeight: 'bold', paddingLeft: '5%' }}>Nội dung công việc :</Text>
            <Text style={{ fontSize: 16, color: 'black', fontWeight: '500', paddingLeft: '5%' }}>  {congViec_id.noiDungCongViec}</Text>

          </ScrollView>

          <View style={{ width: '100%', justifyContent: 'center', alignItems: 'center', }}>
            <Pressable style={{ width: 160, height: 50, backgroundColor: '#B0A4A8', justifyContent: 'center', borderRadius: 25, alignItems: 'center' }} onPress={() => setModalVisible_ctcv(false)}>
